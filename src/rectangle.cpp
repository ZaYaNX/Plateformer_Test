...
void Rect::Move()
{
	...
	{
		body->SetLinearVelocity(b2Vec2(2.5f, body->GetLinearVelocity().y));
	}
	...
	{
		body->SetLinearVelocity(b2Vec2(body->GetLinearVelocity().x, -5.5f));
	}
}
...
void Rect::update(sf::Time dt)
{
	...
	rectShape.setPosition(centerPos - pixelSize / 4.f);
}
